...
import frc.robot.subsystems.Turret.Aiming;
 ...
 class SafeCenterClass implements Runnable{
    ...
    public void run(){
        ...
        while (timer.get() < 1) {
            if (Forward) aiming.angleMotor.set(.6);
            else aiming.angleMotor.set(-.6);
...
public class aimTurret extends CommandBase {
    ...
    float Kp = .04f, Ki = .01f;
    double EndPos = 290, intergralTop, intergralBottom, proportional, intergral, error, desired, lastSeen, defaultThreadCount, distance, angleGoalDegree;
